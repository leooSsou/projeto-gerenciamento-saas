...
* **Autenticação, JWT e Segurança** (Próximo Passo):
  ...
  * **Persistência e Modelagem de Banco (Responsável: Leonardo)**:
    - [x] Mapear os modelos SQLAlchemy físicos de `tenants` e `usuarios` em [src/infrastructure/database/models.py](file:///media/souzza/Dados/projeto_gerenciamento/src/infrastructure/database/models.py) e gerar a migração Alembic.
